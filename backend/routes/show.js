...
router.get('/result', async (req, res) => {
  ...
  const encodedTitle = encodeURI(req.body.shprfnm); // 공연명 검색 시, 문자는 URIEncoding
  const query = {
    // 필수 요청 변수
    ...
    // 선택 요청 변수
    shprfnm: encodedTitle || '', // 값을 엔코드 하고 넣어야한다.
  ...
  try {
    ...
    const showListXML = await axios.get(URL); // XML 파일 데이터
    // JSON으로 변환
    const showList = await xml2js.parseStringPromise(showListXML.data);
